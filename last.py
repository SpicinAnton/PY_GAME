    ...
    for y in range(len(level)):
        for x in range(len(level[y])):
            if level[y][x] == '.':
                ...
                Tile('empty', x, y)
                qskelets.append(GoingSkelet(x, y))
            elif level[y][x] == '+':
                Tile('empty', x, y)
                speedpotion = SpeedPotion(x, y)

    # вернем игрока, а также размер поля в клетках
    return new_player, x, y, speedpotion, portal, qskelets
...
def start_screen():
    intro_text = ["Приветствуем вас это игра ЛАБИРИНТ", "",
                  "Движение осуществяется",
                  "нажатием на стрелочки,",
                  "для перехода на следующий уровень",
                  'зайдите в портал,',
                  'зелье удваивает скорость,',
                  'Не попадитесь скелетам']
...
player, max_x, max_y, speedpotion, portal, qskelets = generate_level(level_map)
move_sk = pygame.USEREVENT + 1
pygame.time.set_timer(move_sk, 50)
teleport = pygame.sprite.collide_rect(player, portal)
...
while run:
    ...
    for event in pygame.event.get():
        ...
        hit = pygame.sprite.spritecollideany(player, potions_group)
        if hit:
            speedpotion.kill()
            speed = 2
        tp = pygame.sprite.collide_rect(player, portal)

        fail = pygame.sprite.spritecollideany(player, moving_sprites)
        if fail:
            uroven -= 1
            tp = True
        ...
        if tp:
            ...
            if uroven == 1:
                ...
                level_map = load_level('mapa1.txt')
                player, max_x, max_y, speedpotion, portal, qskelet = generate_level(level_map)
            ...
            if uroven == 2:
                ...
                player.kill()
                moving_sprites.empty()
                level_map = load_level('mapa2.txt')
                player, max_x, max_y, speedpotion, portal, qskelet = generate_level(level_map)
            ...
            if uroven == 3:
                ...
                player.kill()
                moving_sprites.empty()
                level_map = load_level('mapa3.txt')
                player, max_x, max_y, speedpotion, portal, qskelet = generate_level(level_map)
